...
\title{Supplementary information\\Recovery supervision with calibrated fallback for multi-sensor fusion under available-but-corrupted channels}
\author{Riyang Luo}
\date{}
...
\section{Revised strict estimand, selective recovery and inference boundary}

The revised headline design retains chemical-array batches 8--10, group~1 silent Gaussian corruption, prevalence 0.40, standardized scale 3 and the common fault realization. It changes only the evaluation estimand: an observation is primary only when group~1 is assigned, remains available after independent missingness and therefore receives a nonzero intervention. This gives 1,436 strict observations from 1,765 assignments; 329 assigned-but-unavailable observations are reported separately. Fault indices, missing masks and noise values are identical across methods and seeds. The strict definition and the analysis scripts were frozen before the 10-seed upgrade rerun, but the redefinition followed audit of earlier assignment-based results. Exact Wilcoxon $P$ values therefore describe optimization stability rather than preregistered population inference. A fixed-ensemble interval first resamples acquisition batch and then observations within sampled batch. Every interval uses 5,000 equal-tailed percentile replicates.

\begin{table}[ht]
\centering
\caption{Strict fault-applied-and-available effects under the fixed Gaussian intervention. Values are paired across 10 optimization seeds.}
\resizebox{0.76\textwidth}{!}{\input{tables/major9_strict_estimand.tex}}
\end{table}

The selective-recovery experiment divides the existing Batch~7 calibration partition into two ordered halves. The first fits separate PDRF and RO-PDRF-Full temperatures. The second contains two calibration-only realizations of Gaussian, offset, drift and stuck-at faults and fits a standardized logistic preference model only where base and recovery correctness differ. The 12 features are base confidence, recovery confidence, confidence difference, both normalized entropies and their difference, base--recovery Jensen--Shannon divergence, each model's divergence from leave-one-group-out recovery consensus, each model's class agreement with that consensus, and the fraction of legal group removals. Candidate thresholds are $0.50,0.525,\ldots,0.90$. Balanced maximizes selector-half accuracy and then minimizes negative transfer. Safe minimizes negative transfer and then maximizes accuracy and recovery. Thresholds vary by optimization seed because each independently fitted model pair has different calibrated behavior. Test labels are loaded only after coefficients and thresholds have been frozen.

Across the four test mechanisms and 10 fitted pairs, Balanced preserves 569 of 1,692 recovery opportunities and leaves 294 of 1,375 negative-transfer opportunities. Safe preserves 303 and leaves 137, respectively. These pooled counts contain 57,440 model--fault--observation predictions, not independent physical events. Four-fault aggregate intervals resample fault type and matched optimization seed and remain descriptive because there are only four top-level mechanisms.

\begin{table}[ht]
\centering
\caption{Selective-recovery strict-subset macro-AUROC. Values are means $\pm$ standard deviations across 10 optimization seeds.}
\resizebox{0.94\textwidth}{!}{\input{tables/major9_safe_recovery.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Recovery-oriented modern-baseline audit on the strict subset. RO-MER contains group experts and one joint expert with sample-dependent routing.}
\resizebox{0.80\textwidth}{!}{\input{tables/major9_modern_baseline.tex}}
\end{table}

The following two tables retain the earlier assignment-based base-architecture analysis for reconciliation. They are not substitutes for the strict recovery-objective comparisons above.

\begin{table}[ht]
\centering
\caption{Archived assignment-based paired optimization-seed effects for the original base-architecture comparison. NLL is sign-aligned so that positive values favour PDRF.}
\resizebox{0.76\textwidth}{!}{\input{tables/major_seed_effects_table.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Archived assignment-based fixed-ensemble effects from 5,000 paired resamples. Positive aligned differences favour PDRF.}
\resizebox{0.82\textwidth}{!}{\input{tables/major_bootstrap_table.tex}}
\end{table}

The exact McNemar comparison of ensemble hard decisions counted 270 observations correct only for PDRF and 290 correct only for CAGF ($P=0.422$). The complete CAGF-prediction by PDRF-prediction matrix is stored in \path{source_data/major_prediction_disagreement.csv}. Mean per-seed and ensemble accuracies are stored in \path{source_data/major_seed_ensemble_explanation.json}. Averaging class probabilities can change the winning class, so mean single-model accuracy and ensemble accuracy are not the same estimand.
...
\section{Independent hydraulic validation}
...
\section{Real-operational AHU sensor-fault validation}
...
The supplied Python scripts reproduce data processing, model fitting, targeted extensions, statistical analysis, tables and figures. The upgrade-specific entry points are \path{analyse_major9_estimand.py}, \path{run_major9_safe_recovery.py}, \path{run_major9_modern_baseline.py}, \path{analyse_major9_upgrade.py} and \path{make_major9_figure.py}. The frozen design is stored in \path{source_data/major9_frozen_protocol.json}; prediction-level outputs, selector thresholds, safety accounting, model costs and bootstrap summaries are stored under \path{source_data/major9_*}. Figures use Python/matplotlib exclusively. Editable SVG/PDF figures and a 600-dpi TIFF are included in the manuscript package. Script names and execution order are listed in the package README.

The chemical-array experiment uses real longitudinal measurements with controlled feature-level sensor faults. The hydraulic experiment uses native test-rig component-condition labels with controlled sensor faults. The AHU audit adds naturally observed operational records with expert-annotated sensor-fault classes from 41 installed AHUs across three buildings. Those labels were assigned from engineering rules and expert review rather than maintenance-confirmed sensor replacement records. The evidence therefore supports field-operational fault recognition but not hardware-failure prognosis or zero-shot building transfer.

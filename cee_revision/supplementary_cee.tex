...
Uncertainty for the selector audit uses 1,000 crossed-cluster bootstrap replicates. Fault mechanism and fitted model-pair seed are independently sampled with replacement; the same sampled set of 10 seed identities is retained across every sampled mechanism, after which strict observations are resampled within each fault-by-seed cell. Retaining the seed draw across faults represents reuse of one fitted endpoint pair and avoids incorrectly nesting seed under mechanism. The resulting intervals quantify variation over the four tested mechanisms, repeated fitting and observations. They do not treat 57,440 repeated model--fault--observation predictions as independent physical events, and they do not represent a population of devices.
...
\begin{table}[ht]
\centering
\caption{Selector feature-set ablation. Calibration discrimination uses out-of-fold disagreement rows; strict-test quantities first average mechanisms within fitted seed.}

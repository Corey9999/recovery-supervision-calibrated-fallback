...
The Q1 revision adds frozen run CEE-CF10-R2-LITE. It reuses the chemical split, fixed test realization, optimization seeds 101--110 and training-only preprocessing of CEE-CF10-R2, but routes between PDRF and RO-PDRF-Lite. Six confidence/entropy features are calculated from temperature-scaled endpoint probabilities. The router emits the selected temperature-scaled probability vector, and no post-routing recalibration is performed. The primary selector uses $L_2$-regularized logistic regression with $C=0.05$ and requires two endpoint passes.
...
\begin{table}[ht]
\centering
\caption{Severity audit against the empirical training distribution. Percentile shifts are absolute changes in empirical cumulative probability. The formal scale-3 setting is a severe stress test.}
...
The Q1 entry points are \path{run_cee_lite_routing_validation.py}, \path{analyse_cee_lite_results.py}, \path{analyse_cee_q1_scores.py}, \path{analyse_cee_fault_plausibility.py} and \path{make_cee_q1_figures.py}. The machine-readable frozen design is \path{cee_cf10_r2_lite_config.json}. Exact local package versions are recorded in \path{requirements-lock.txt} and \path{environment.yml}. Prediction-level outputs, selector thresholds, safety accounting, model costs, probability metrics and fixed-mechanism/batch intervals are stored under \path{source_data/q1_*}. Figures use Python/matplotlib exclusively. Editable SVG/PDF figures and 600-dpi TIFFs are included. The versioned release archive contains a SHA-256 manifest, and the public repository URL and exact commit are reported in the main article.

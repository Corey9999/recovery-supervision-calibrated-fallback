...
\title{Supplementary information\\Support-gated selective recovery under controlled multi-sensor corruption}
\author{Riyang Luo}
\date{}
...
\resizebox{0.72\textwidth}{!}{\input{tables/cee_recovery_eligibility.tex}}
\end{table}
...
\resizebox{0.72\textwidth}{!}{\input{tables/q1_response_audit.tex}}
\end{table}
...
\resizebox{0.82\textwidth}{!}{\input{tables/major_compute_table.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/q1_result_manifest.tex}}
\end{table}

The deployment-support gate was proposed after the frozen experiment in response to review and was applied retrospectively to the 10 frozen endpoint pairs. It was not used to select or replace the ungated headline result. Under the conservative reference configuration, a fitted pair is operationally supported only if its calibration data contain at least 25 base-better and 25 recovery-better events, every grouped fitting fold contains both preference classes, the median out-of-fold AUROC across 10 repeated grouped splits is at least 0.65, no repeated AUROC is below 0.60 and the repeated threshold range is at most 0.10. All conditions must pass. The intended prospective rule is that a site failing the gate emits PDRF and continues collecting calibration evidence; coefficients and thresholds are not transported from another pair or site.

\begin{table}[ht]
\centering
\caption{Post hoc proposed deployment-support gate applied retrospectively to the 10 frozen endpoint pairs. Event counts use informative Batch~7 calibration rows. AUROC and threshold summaries use 10 repeated grouped cross-fitting splits. The gate is an intended prospective operational boundary, not a prespecified test or replacement estimand.}
\label{tab:supp-deployment-support}
\resizebox{\textwidth}{!}{\input{tables/strict88_deployment_support.tex}}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Descriptive consequence of applying the post hoc support gate retrospectively to the frozen strict test set. Unsupported endpoint pairs emit PDRF on every observation; supported pairs retain their previously frozen Lite-CF selector.}
\label{tab:supp-support-policy}
\resizebox{0.88\textwidth}{!}{\input{tables/strict88_support_gated_policy.tex}}
\end{table}

The reference values 25/0.65/0.10 are not treated as uniquely correct. We crossed minimum event counts 15, 25 and 40, median repeated AUROC thresholds 0.60, 0.65 and 0.70, and maximum threshold ranges 0.05, 0.10 and 0.15. The requirement that every fitting fold contain both preference classes and the minimum repeat AUROC of 0.60 remained fixed. Across the 27 configurations, 1--7 fitted pairs passed, prevention ranged from 95.2\% to 97.9\%, retention from 1.4\% to 8.6\%, equal-cost utility from $+0.35$ to $+13.75$ per 10,000 and fitted-pair worst-batch utility from $-3.17$ to $+0.43$. Several settings produced identical decisions because they admitted the same fitted pairs or added a pair whose frozen selector changed selection without changing correction or harm counts.

\begin{table}[H]
\centering
\caption{Complete $3\times3\times3$ sensitivity grid for the post hoc deployment-support gate. $U_1$ assigns equal value to a correction and a harm with no compute penalty. Worst-batch utility first takes the least favourable acquisition-batch mean within each fitted pair and then averages the 10 fitted-pair values. The bold count marks the conservative reference configuration.}
\label{tab:supp-gate-grid}
\scriptsize
\resizebox{0.90\textwidth}{!}{\input{tables/round2_gate_sensitivity_full.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/q1_lite_stream_quality.tex}}
\end{table}
...
\resizebox{0.82\textwidth}{!}{\input{tables/q1_lite_agreement_quality.tex}}
\end{table}
...
\resizebox{0.94\textwidth}{!}{\input{tables/q1_lite_probability_dispersion.tex}}
\end{table}

Repeated grouped cross-fitting uses 10 split seeds for each of 10 fitted endpoint pairs. All corruption realizations derived from one calibration observation remain in one fold. Across the resulting 100 fits, the arithmetic mean of the 100 individual seed--repeat AUROCs (the mean-of-repeats AUROC) was $0.720\pm0.138$ and ranged from 0.327 to 0.954. It is distinct from the pooled-row AUROC of 0.562 obtained by concatenating the formal out-of-fold scores over informative calibration rows. Repeated thresholds ranged from 0.525 to 0.90. The mean primary-objective tie count was 1.78 and the maximum was 9. The formal once-per-pair fit had 1.5 tied thresholds on average (range 1--4). Ties are broken by the highest threshold after realized harms, selected accuracy and retained corrections are equal.
...
\resizebox{0.74\textwidth}{!}{\input{tables/q1_lite_coefficient_stability.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Lite-CF regularization path. Values average five grouped repeats for each of 10 endpoint pairs. The original router setting $C=0.05$ was selected before the post hoc support gate was proposed; it is a strong-shrinkage choice rather than the calibration-discrimination optimum.}
\resizebox{0.72\textwidth}{!}{\input{tables/q1_lite_regularization.tex}}
\end{table}
...
\resizebox{0.70\textwidth}{!}{\input{tables/q1_lite_calibration_size.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/q1_lite_batch_intervals.tex}}
\end{table}
...
\section{Major-review all-row, utility and transport audits}

The new major-review analyses reuse the frozen PDRF/RO-PDRF-Lite predictions whenever possible. The scale/group transport script independently refits the same ten endpoint pairs only to generate missing prediction-level files. Its formal scale-3/group-1 output contains the same 57,440 strict rows as the frozen run. Endpoint argmax predictions agree for every row; the maximum absolute probability difference is $1.11\times10^{-6}$, and calibration features and correctness labels match exactly. This numerical check is completed before transport results are used.

For each fitted pair, the all-row outcome is 0 for base better, 1 for endpoint-equivalent correctness and 2 for recovery better. Calibration means are 36.1, 1,771.2 and 53.6 rows, respectively. Five-fold stratified group cross-fitting keeps all fault realizations from an original observation together. The multinomial score is $p_2-p_0$. The two-stage score is $p(b\ne r\mid\mathbf x)\{2p(r\text{ better}\mid b\ne r,\mathbf x)-1\}$. Its two factors are fitted in separate grouped pipelines. These models do not require inverse-probability weighting because outcomes are observed on every calibration row; the issue is class sparsity, not missing labels.

The complete Pareto audit ranks scores separately within each fixed test mechanism--fitted-pair cell at selection fractions 0, 0.025, $\ldots$, 1 and uses test labels to calculate prevention and retention. These are descriptive cell-stratified test ranking curves, not deployable operating policies, and they never select a submitted threshold. Calibration-matched comparisons instead choose scalar/tree/random thresholds using out-of-fold calibration prevention or retention and apply them once to test. The output files distinguish the matching target from the realized test rate, because transport prevents exact matching. The empirical dominance file checks every averaged comparator point against the frozen operating point; no comparator has both at least 94.7\% prevention and at least 9.7\% retention with one strict inequality.

Application-specific thresholds maximize $n_C-rn_H$ on out-of-fold calibration predictions for $r\in\{1,2,5,10\}$. At test ratio 1, all-row/two-stage mean utilities are $+107.9/+110.6$ per 10,000, below always recovery ($+132.1$). At ratio 2, their means are $+30.8/+37.8$, above always recovery ($+15.5$). The two-stage 95\% fitted-pair interval is $-2.7$ to $+78.3$, 7/10 fitted pairs are positive and the mean test-minus-calibration utility is $-99.0$. At ratios 5 and 10 every non-base policy has negative mean utility. This is a fixed-test analytical comparison, not a deployment recommendation.

Only conditional Lite-CF has the retrospectively evaluated pair-level support gate. A conditional fit that fails it falls directly back to PDRF and cannot switch automatically to all-row or two-stage routing. The all-row and two-stage variants remain calibration-frozen analytical controls because no independent model-specific support gates were defined for them. Prospective deployment would require a separately frozen support rule for the chosen router family, followed by cost-specific threshold selection; failure at the support stage would revert to PDRF.

\begin{table}[H]
\centering
\caption{Complete fitted-pair dispersion and decision metrics for endpoint, safety and utility-selected policies. Each fitted-pair row first pools the four fixed mechanisms. Utilities are per 10,000 strict observations with $V_C=1$ and $\lambda=0$. Calibration utility and test-minus-calibration change are available only for the three utility-selected router families. Because decision-event denominators are pooled within fitted pair before averaging, decision percentages can differ slightly from the equal-weight mechanism--pair headline means.}
\label{tab:supp-utility-dispersion}
\scriptsize
\resizebox{\textwidth}{!}{\input{tables/round2_utility_fitted_pair_full.tex}}
\end{table}

Severity transport crosses calibration and test scales 1, 2 and 3. Sensor-group transport calibrates only on group~1 and evaluates all four groups at scale 3. Mechanism transport includes leave-one-mechanism-out, Gaussian-only and five family-mixture calibrations. The family-mixture audit resamples a fixed total calibration size with either equal 25\% weights or one mechanism at 70\% and the remaining mechanisms at 10\% each. These are selector-shift audits; endpoint training is unchanged.

Temperature-feature sensitivity reconstructs uncalibrated endpoint probabilities from scaled probabilities and fitted temperatures using $p_c^{\mathrm{raw}}\propto(p_c^{\mathrm{scaled}})^T$. Platt and isotonic mappings fit only out-of-fold conditional scores. Platt mapping is monotone and leaves frozen decisions unchanged after threshold mapping; isotonic plateaus create ties and can change decisions. Rank-normalized and class-conditional logistic controls use the same calibration rows. The output reports preference AUROC, NLL, Brier score and 10-bin ECE separately from emitted-probability metrics.

The complete mechanism$\times$batch$\times$method matrix has one row per fitted pair, mechanism, acquisition batch and policy. It includes accuracy, macro-AUROC, correction and harm opportunities, realized correction and harm counts, prevention, retention, selection and utility. Batch summaries are reported as pooled observation-weighted performance, equal-batch means, fitted-pair-specific worst batch and pooled Batches~8--9 versus Batch~10. All $t$ intervals are labelled fitting-variation intervals and are accompanied by medians, interquartile ranges and full ranges in the CSV source data.
...
\resizebox{0.84\textwidth}{!}{\input{tables/cee_run_manifest.tex}}
\end{table}
...
\resizebox{0.92\textwidth}{!}{\input{tables/cee_selector_parameters.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/cee_selector_seed_diagnostics.tex}}
\end{table}
...
\resizebox{0.76\textwidth}{!}{\input{tables/cee_selector_reliability.tex}}
\end{table}
...
\resizebox{0.78\textwidth}{!}{\input{tables/cee_balanced_sensitivity.tex}}
\end{table}
...
\resizebox{0.92\textwidth}{!}{\input{tables/q1_lite_cpu_latency.tex}}
\end{table}
...
\resizebox{0.86\textwidth}{!}{\input{tables/strict88_cpu_thread_summary.tex}}
\end{table}
...
\resizebox{0.86\textwidth}{!}{\input{tables/cee_compute_cost.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Counted linear-layer FLOPs and selector forward passes as the number of groups grows.}
\resizebox{0.62\textwidth}{!}{\input{tables/cee_group_scaling.tex}}
\end{table}
...
\resizebox{0.76\textwidth}{!}{\input{tables/q1_strict_class_batch_counts.tex}}
\end{table}
...
\resizebox{0.88\textwidth}{!}{\input{tables/q1_fault_severity.tex}}
\end{table}
...
\resizebox{0.84\textwidth}{!}{\input{tables/q1_fault_examples.tex}}
\end{table}
...
\resizebox{0.88\textwidth}{!}{\input{tables/major_subset_table.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Results reported separately for acquisition batches 8, 9 and 10. Values are means across 10 seeds.}
\resizebox{0.72\textwidth}{!}{\input{tables/major_batch_table.tex}}
\end{table}
...
\resizebox{0.76\textwidth}{!}{\input{tables/major_class_table.tex}}
\end{table}
...
\resizebox{0.66\textwidth}{!}{\input{tables/major_quality_table.tex}}
\end{table}
...
\resizebox{0.90\textwidth}{!}{\input{tables/major_grouping_distribution_table.tex}}
\end{table}
...
\resizebox{0.65\textwidth}{!}{\input{tables/major_logo_table.tex}}
\end{table}
...
\resizebox{0.94\textwidth}{!}{\input{tables/strict88_severity_comparison.tex}}
\end{table}
...
\resizebox{0.70\textwidth}{!}{\input{tables/major_hyperparameter_table.tex}}
\end{table}
...
\begin{minipage}{0.47\textwidth}\centering\resizebox{\textwidth}{!}{\input{tables/hydraulic_counts_cooler.tex}}\end{minipage}\hfill
\begin{minipage}{0.47\textwidth}\centering\resizebox{\textwidth}{!}{\input{tables/hydraulic_counts_valve.tex}}\end{minipage}
\end{table}

\begin{table}[ht]
\centering
\caption{Hydraulic pump and accumulator condition values and split counts.}
\begin{minipage}{0.47\textwidth}\centering\resizebox{\textwidth}{!}{\input{tables/hydraulic_counts_pump.tex}}\end{minipage}\hfill
\begin{minipage}{0.47\textwidth}\centering\resizebox{\textwidth}{!}{\input{tables/hydraulic_counts_accumulator.tex}}\end{minipage}
\end{table}
...
\resizebox{0.78\textwidth}{!}{\input{tables/hydraulic_effects_table.tex}}
\end{table}
...
\resizebox{0.94\textwidth}{!}{\input{tables/major5_teacher_conditional_rates.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/modern_uncertainty_baseline.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major6_strong_controls.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major8_literature_teacher_controls.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major8_teacher_safeguards.tex}}
\end{table}
...
\resizebox{0.98\textwidth}{!}{\input{tables/major8_qmf_implementation.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major8_qmf_sensitivity.tex}}
\end{table}
...
\label{tab:supp-cost}
\resizebox{\textwidth}{!}{\input{tables/recovery_cost.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major8_training_costs.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major7_fault_type_ema.tex}}
\end{table}
...
\resizebox{0.92\textwidth}{!}{\input{tables/major8_fault_type_architecture.tex}}
\end{table}
...
\label{tab:supp-lofo}
\resizebox{0.84\textwidth}{!}{\input{tables/major8_leave_one_fault_type_out.tex}}
\end{table}
...
\resizebox{0.72\textwidth}{!}{\input{tables/major7_ema_decay.tex}}
\end{table}
...
\resizebox{0.92\textwidth}{!}{\input{tables/q1_sensitivity_table.tex}}
\end{table}
...
\resizebox{0.82\textwidth}{!}{\input{tables/q1_hydraulic_extension_table.tex}}
\end{table}
...
\resizebox{0.72\textwidth}{!}{\input{tables/ahu_temporal_splits.tex}}
\end{table}
...
\resizebox{0.72\textwidth}{!}{\input{tables/ahu_class_distribution.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/ahu_temporal_detailed.tex}}
\end{table}
...
\resizebox{0.86\textwidth}{!}{\input{tables/ahu_subtype_recall.tex}}
\end{table}
...
\resizebox{0.76\textwidth}{!}{\input{tables/ahu_ensemble_confusion.tex}}
\end{table}
...
\resizebox{0.72\textwidth}{!}{\input{tables/ahu_temporal_tests.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/ahu_conformal.tex}}
\end{table}
...
\resizebox{0.78\textwidth}{!}{\input{tables/ahu_cross_building_exploratory.tex}}
\end{table}
...
\resizebox{0.76\textwidth}{!}{\input{tables/major3_ahu_shift_distance.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major3_hospital_feature_shift.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major4_protocol_reconciliation.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major3_2x2_effects.tex}}
\end{table}
...
\resizebox{0.90\textwidth}{!}{\input{tables/major3_2x2_cost.tex}}
\end{table}
...
\resizebox{0.84\textwidth}{!}{\input{tables/major3_teacher_ablation.tex}}
\end{table}
...
\resizebox{0.82\textwidth}{!}{\input{tables/major3_loss_ablation.tex}}
\end{table}
...
\resizebox{0.78\textwidth}{!}{\input{tables/major4_regularizer_sensitivity.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major7_consistency_ladder_effects.tex}}
\end{table}
...
\resizebox{0.88\textwidth}{!}{\input{tables/major5_fault_realization_effects.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{RO-PDRF-Full minus RO-CAGF affected-AUROC by acquisition batch across fault realizations and seeds.}
\resizebox{0.76\textwidth}{!}{\input{tables/major5_batch_stratified_effects.tex}}
\end{table}
...
\resizebox{0.65\textwidth}{!}{\input{tables/major5_teacher_quadrants.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Teacher-confidence audit. $\Delta p_y$ is RO-PDRF-Full minus PDRF true-class probability on the faulted observation.}
\resizebox{0.95\textwidth}{!}{\input{tables/major5_teacher_confidence.tex}}
\end{table}
\FloatBarrier
...
\resizebox{0.58\textwidth}{!}{\input{tables/major5_hydraulic_blocked.tex}}
\end{table}
...
\resizebox{0.92\textwidth}{!}{\input{tables/major3_2x2_hydraulic.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Nearest-training-cycle audit in standardized 240-dimensional hydraulic feature space.}
\resizebox{0.78\textwidth}{!}{\input{tables/major5_hydraulic_similarity.tex}}
\end{table}
\FloatBarrier
...
\resizebox{0.94\textwidth}{!}{\input{tables/major5_ensemble_calibration.tex}}
\end{table}
...
\resizebox{0.86\textwidth}{!}{\input{tables/major5_complexity_scaling.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Training stability for clean-distillation controls. Gradient norms are measured before clipping at 5.}
\resizebox{0.84\textwidth}{!}{\input{tables/major5_training_stability.tex}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/major_gas_results.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Original formal-run mechanism ablation.}
\resizebox{0.78\textwidth}{!}{\input{tables/major_ablation_table.tex}}
\end{table}

\begin{table}[ht]
\centering
\caption{Original formal-run recovery and calibration results. Aff. denotes the affected subset.}
\resizebox{\textwidth}{!}{\input{tables/q1_recovery_results.tex}}
\end{table}
...
The Q1 entry points are \path{run_cee_lite_routing_validation.py}, \path{analyse_cee_lite_results.py}, \path{analyse_cee_q1_scores.py}, \path{analyse_cee_fault_plausibility.py}, \path{run_cee_strict88_additional_audits.py}, \path{analyse_cee_strict88_revision.py}, \path{analyse_cee_major_router_audits.py}, \path{run_cee_major_severity_transport.py}, \path{analyse_cee_major_shift_audits.py}, \path{analyse_cee_round2_revision.py}, \path{make_cee_q1_figures.py} and \path{make_cee_major_revision_figures.py}. The machine-readable frozen design is \path{cee_cf10_r2_lite_config.json}; added designs are \path{source_data/strict88_additional_audits_design.json}, \path{source_data/major_router_audit_design.json} and \path{source_data/major_shift_design.json}. Exact local package versions are recorded in \path{requirements-lock.txt} and \path{environment.yml}. Prediction-level outputs, selector thresholds, full Pareto curves, all-row probabilities, matched controls, utility thresholds, mechanism/severity/group/batch transport, engineering event efficiency, fixed-thread timings, 27 support-gate configurations and fitted-pair utility dispersion are stored under \path{source_data/q1_*}, \path{source_data/strict88_*}, \path{source_data/major_*} and \path{source_data/round2_*}. The routing API emits the selected probability vector by default and, when requested, also returns endpoint identity and the conditional preference score. Figures use Python/matplotlib exclusively. Editable SVG/PDF figures and 600-dpi TIFFs are included. All legacy selector baselines, negative analyses and superseded estimands remain auditable. The versioned release archive contains a SHA-256 manifest, and the public repository URL and exact commit are reported in the main article.

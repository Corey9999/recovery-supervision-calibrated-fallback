...
The complete Pareto audit ranks test rows within each fixed mechanism--fitted-pair cell at selection fractions 0, 0.025, $\ldots$, 1. Curves are descriptive and never select a submitted threshold. Calibration-matched comparisons choose scalar/tree/random thresholds using out-of-fold calibration prevention or retention and apply them once to test. The output files distinguish the matching target from the realized test rate, because transport prevents exact matching. The empirical dominance file checks every averaged comparator point against the frozen operating point; no comparator has both at least 94.7\% prevention and at least 9.7\% retention with one strict inequality.

Application-specific thresholds maximize $n_C-rn_H$ on out-of-fold calibration predictions for $r\in\{1,2,5,10\}$. At test ratio 1, all-row/two-stage utilities are $+107.9/+110.6$ per 10,000, below always recovery ($+132.1$). At ratio 2, they are $+30.8/+37.8$, above always recovery ($+15.5$). At ratios 5 and 10 every non-base policy is negative. The submitted analysis therefore reports a policy switch rather than extrapolating the ratio-1 result.
...
The complete mechanism$\times$batch$\times$method matrix has one row per fitted pair, mechanism, acquisition batch and policy. It includes accuracy, macro-AUROC, correction and harm opportunities, realized correction and harm counts, prevention, retention, selection and utility. Batch summaries are reported as pooled observation-weighted performance, equal-batch means, worst batch and pooled Batches~8--9 versus Batch~10. All $t$ intervals are labelled fitting-variation intervals and are accompanied by medians, interquartile ranges and full ranges in the CSV source data.
